...
def get_url_or_cache(url):
    ...
    if not cacheEnable or (cacheEnable and not os.path.isfile(localFile)):
        # Download web page
...
def scrap_company(company_link, max_reviews=-1, delay_min=0.5, delay_max=1, max_req=1):
...
def scrap_category(category_link, location=False, numberofreviews=0, status="all", timeperiode=0,
    ...
    global task_label, task_progress, task_progress_max
    task_label = "Scraping"
    task_progress = 0
    if (max_companies==-1 or max_reviews==-1):
        task_progress_max = -100
    else:
        task_progress_max = int(max_companies/20)+1 + (int(max_reviews/20)+1) * max_companies

    companies = scrap_companies_from_category_link(category_link, location=location, numberofreviews=numberofreviews,
                                                   status=status,
    ...
    while i < size:
        ...
        # 	i += 1
        # 	continue
        new_dict_company, new_dict_reviews = scrap_company(companies["link"][i], max_reviews=max_reviews,
                                                           delay_min=delay_min,
                                                           delay_max=delay_max, max_req=max_req)
